...
function Request(input, init) {
	...
	this.counter = init.counter || input.follow || 0; // XXX unused in request()
	this.timeout = init.timeout || input.timeout || 0; // XXX unused in request()
	this.gzip = init.gzip !== undefined ?
		init.gzip : input.gzip !== undefined ?
		input.gzip : true;
	this.size = init.size || input.size || 0; // XXX unused in request()
	this.agent = init.agent || input.agent; // XXX unused in request()
